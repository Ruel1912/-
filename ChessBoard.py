...
class ChessBoard:
    ...
    def get_all_pieces(self):
        """Вернуть список всех фигур на доске"""
        pieces = []
        for x in range(8):
            for y in range(8):
                if self.board[x][y]:
    ...
    def get_curr_player_pieces(self):
        """Возвращает все оставшиеся фигуры текущего игрока"""
        pieces = []
    ...
    def get_poss_moves_for(self, piece):
        """Вернуть все возможные ходы для активной фигуры"""
        # ПРИМЕЧАНИЕ: Необходимо проверить ход, если выбран король
        if piece.name != 'King':
    ...
    def move_piece(self, piece, new_position):
        """Перемещает фигуру в новое положение И меняет ее на истину
        Возвращает фигуру, если фигура взята, нет, если нет"""
        pos = piece.position
    ...
    def non_permanent_move(self, piece, new_position):
        """Перемещается фигура и не меняется, переместилась в истину. Используется для проверки шахов/матов"""
        pos = piece.position
        piece.move(new_position)
        self.board[new_position[0]][new_position[1]] = piece
        self.board[pos[0]][pos[1]] = None
    ...
    def castle_king(self, king, new_king_position):
        """Рокировка"""
        # Новая позиция короля = (2, 0), (6, 0), (2, 7), (6, 7)
    ...
    def non_permanent_castle_king(self, king, new_king_position):
        ...
        перемещает соответствующую ладью. НЕ меняется фигура сдвинулся"""
    ...
    def king_in_check(self, king, b):
        ...
        if l_diag[0] >= 0:
            if b[l_diag[0]][l_diag[1]] and b[l_diag[0]][l_diag[1]].name == 'Pawn' and b[l_diag[0]][
                l_diag[1]].color != self.curr_player:
                return True
        if r_diag[0] <= 7:
            if b[r_diag[0]][r_diag[1]] and b[r_diag[0]][r_diag[1]].name == 'Pawn' and b[r_diag[0]][
                r_diag[1]].color != self.curr_player:
                return True
